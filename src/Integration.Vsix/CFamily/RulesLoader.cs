...
namespace SonarLint.VisualStudio.Integration.Vsix.CFamily
{
    ...
    internal class RulesLoader
    {
        ...
        public static List<string> ReadRulesList()
        {
            ...
            string path = Path.Combine(extensionFolder, "CFamily", "RulesList.json");
        ...
        {
            var extensionFolder = Path.GetDirectoryName(typeof(RulesLoader).Assembly.Location);
            string path = Path.Combine(extensionFolder, "CFamily", "Sonar_way_profile.json");
            using (StreamReader r = new StreamReader(path, Encoding.UTF8))
            {
                string json = r.ReadToEnd();
        ...
        {
            var extensionFolder = Path.GetDirectoryName(typeof(RulesLoader).Assembly.Location);
            string path = Path.Combine(extensionFolder, "CFamily", ruleKey + "_params.json");
            if (!File.Exists(path))
            {
