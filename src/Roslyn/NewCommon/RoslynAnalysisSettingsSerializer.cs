...
namespace SonarLint.VisualStudio.Roslyn.Suppressions.NewCommon
{
    ...
    /// </summary>
    public class RoslynAnalysisSettingsSerializer
    {
        private readonly IFileSystem fileSystem;
        private readonly ILogger logger;

        public RoslynAnalysisSettingsSerializer(IFileSystem fileSystem, ILogger logger)
        {
            this.fileSystem = fileSystem ?? throw new ArgumentNullException(nameof(fileSystem));
            this.logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public RoslynAnalysisSettings SafeLoad(string filePath)
        {
            RoslynAnalysisSettings settings = null;
            ...
            else
            {
                try
                {
                    ...
                    settings = JsonConvert.DeserializeObject<RoslynAnalysisSettings>(data);
                }
